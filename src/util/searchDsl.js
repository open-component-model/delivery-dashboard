...
function parseComparisons(query) {
  const components = []
  const re = /(^|\s)(-?)([A-Za-z_][\w.]*?)\s*(>=|<=|==|!=|>|<)\s*("[^"]*"|'[^']*'|[^\s]+)/g
  const rest = (query || '').replace(re, (m, ws, neg, field, op, value) => {
    components.push({ neg: neg === '-', field, op, value: stripQuotes(value) })
    return ws
  })
...
export function parseQueryToCriteria(query, { allowedFields, rangeFields = [] }) {
  ...
  const { rest: q1, components } = parseComparisons(q0)
  ...
  const addArtefactMetadata = (attr, obj) => add({ type: 'artefact-metadata', attr, ...obj })
  ...
  for (const field of options.keywords) {
    ...
    if (asRangeObj && typeof asRangeObj === 'object' && asRangeObj.from !== undefined && asRangeObj.to !== undefined) {
      if (field === 'ocm') {
        ...
        addArtefactMetadata(field, { op: 'range', gte: asRangeObj.from, lte: asRangeObj.to })
      }
      continue
    }
    ...
    if (values.length > 1) addArtefactMetadata(field, { op: 'in', values })
    else addArtefactMetadata(field, { op: 'eq', value: values[0] })
  }
  ...
  if (parsed.exclude) {
    for (const [field, v] of Object.entries(parsed.exclude)) {
      ...
      if (values.length > 1) addArtefactMetadata(field, { op: 'in', values, mode: 'exclude' })
      else addArtefactMetadata(field, { op: 'eq', value: values[0], mode: 'exclude' })
    }
  }
